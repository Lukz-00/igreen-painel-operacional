...
export function edpCruzar(dfBko, dfRec, dfBol, onLog) {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // FALTA NA PAGADORIA → REC sem corresponde na BOL por UC+Mês
  // Identifica boletos que existem nos Recebíveis internos mas não
  // aparecem na Pagadoria EDP — possível boleto não emitido pela EDP.
  // ────────────────────────────────────────────────────────────────────────────
  const faltaNaPagadoria = []

  if (dfBol) {
    // Índice BOL completo por UC+Mês (todos os status)
    const idxBolAll = {}
    for (const bol of dfBol) {
      const inst   = normUC(getField(bol, ['Número da Instalação', 'Numero da Instalacao']))
      const bolMes = normMes(getField(bol, ['Mês de Referência', 'Mes de Referencia']))
      if (!inst || !bolMes) continue
      const k = `${inst}|${bolMes}`
      idxBolAll[k] = true
    }

    // Percorrer todos os REC e verificar quais não têm par na PAG
    for (const rec of dfRec) {
      const inst   = normUC(getField(rec, ['Instalacao', 'Instalação']))
      const recMes = normMes(getField(rec, ['Data Referencia', 'DataReferencia']))
      if (!inst || !recMes) continue
      const k = `${inst}|${recMes}`
      if (idxBolAll[k]) continue  // tem par na PAG — ok

      const bko  = findBko(inst, normUC(getField(rec, ['Numero Cliente', 'NumeroCliente'])), normUC(getField(rec, ['Codigo Cliente', 'CodigoCliente'])))
      const st   = (getField(rec, ['Status']) || '').toUpperCase().trim()
      const stFin = (getField(rec, ['Status Financeiro Cliente']) || '').trim()

      faltaNaPagadoria.push({
        'UC':                   inst,
        'Cliente':              bko ? getField(bko, ['Nome']) : getField(rec, ['Cliente']),
        'Tem Data Ativo':       bko ? 'SIM' : 'NÃO',
        'Mês Referência':       recMes,
        'Status Recebíveis':    st,
        'Status Fin. REC':      stFin,
        'Valor':                fmtValor(getField(rec, ['Valor A Pagar', 'ValorAPagar'])),
        'Vencimento REC':       getField(rec, ['Data Vencimento']),
        'URL Boleto':           getField(rec, ['Url Boleto', 'UrlBoleto']),
        'Linha Digitável':      getField(rec, ['Linha Digitavel']),
        'Motivo':               'UC+Mês presente nos Recebíveis sem correspondência na Pagadoria EDP',
        _rawRec: rec,
        _rawBko: bko || null,
      })
    }
    log(`Falta na Pagadoria (REC sem par BOL): ${faltaNaPagadoria.length.toLocaleString('pt-BR')}`, 'ok')
  }
  ...
  // ── Auditoria: reconciliação numérica BOL × REC ──────────────────────────────
  const totalBolRows  = dfBol?.length || 0
  const totalRecRows  = dfRec.length
  const emAmbosCount  = totalBolRows - (faltaNaPagadoria.length > 0 ? 0 : 0) // aproximado
  const soNaRec       = faltaNaPagadoria.length

  // Boletos na PAG sem par no REC (cálculo inverso)
  let soNaPag = 0
  if (dfBol) {
    const idxRecAll2 = {}
    for (const rec of dfRec) {
      const inst   = normUC(getField(rec, ['Instalacao', 'Instalação']))
      const recMes = normMes(getField(rec, ['Data Referencia', 'DataReferencia']))
      if (inst && recMes) idxRecAll2[`${inst}|${recMes}`] = true
    }
    for (const bol of dfBol) {
      const inst   = normUC(getField(bol, ['Número da Instalação', 'Numero da Instalacao']))
      const bolMes = normMes(getField(bol, ['Mês de Referência', 'Mes de Referencia']))
      if (inst && bolMes && !idxRecAll2[`${inst}|${bolMes}`]) soNaPag++
    }
  }
  const taxaCobertura = totalRecRows > 0
    ? (((totalRecRows - soNaRec) / totalRecRows) * 100).toFixed(1)
    : '0'

  const auditoria = {
    totalBol: totalBolRows,
    totalRec: totalRecRows,
    soNaRec,
    soNaPag,
    taxaCobertura,
  }
  ...
  return {
    ...
    metrics: {
      ...
      faltaNaPagadoria: faltaNaPagadoria.length,
