...
// ── Gráfico de Barras ─────────────────────────────────────────────────────────
function MesesChart({ rows }) {
  const { porMes, maxMes } = useMemo(() => {
    if (!rows.length) return { porMes: [], maxMes: 1 }
    const contagem = {}
    rows.forEach(r => {
      const mes = String(r[MES_COL] || '—').trim()
      contagem[mes] = (contagem[mes] || 0) + 1
    })
    const ordenados = Object.entries(contagem).sort((a, b) => a[0].localeCompare(b[0]))
    const max = Math.max(...Object.values(contagem), 1)
    return { porMes: ordenados, maxMes: max }
  }, [rows])

  if (!rows.length) return null

  return (
    <div className="bg-s2 border border-bd rounded-xl p-4">
      <div className="text-[12px] font-semibold text-tx mb-3 uppercase tracking-widest">
        Divergentes por Mês de Referência
      </div>
      <div className="space-y-2">
        {porMes.map(([mes, cnt]) => {
          const w = Math.max(Math.round((cnt / maxMes) * 100), 2)
          return (
            <div key={mes} className="flex items-center gap-3">
              <div className="w-16 text-right text-[11px] text-tx3 font-medium flex-shrink-0">{mes}</div>
              <div className="flex-1 bg-s1 rounded-md h-[22px] overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-acc to-purple-500 rounded-md flex items-center justify-end pr-2 text-[10px] font-bold text-white transition-all duration-500"
                  style={{ width: `${w}%` }}
                >
...
export function DivergentesTable({ rows = [] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Gráfico de barras (reativo aos filtros) */}
      {filtered.length > 0 && <MesesChart rows={filtered} />}
